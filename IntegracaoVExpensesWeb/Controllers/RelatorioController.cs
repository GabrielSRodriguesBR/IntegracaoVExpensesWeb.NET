    ...
    public class RelatorioController : Controller
    {
        ...
        public ActionResult Index(RelatorioFilterViewModel filter)
        {
            ...
            if (!string.IsNullOrEmpty(filter.DataInicio) && !string.IsNullOrEmpty(filter.DataFim))
            {
                ...
                query = query.Where(r => r.DataAprovacao >= dataInicio && r.DataAprovacao <= dataFim);
            }
        ...
        {
            var query = db.Relatorios.Include(s => s.Despesas).Where(s => s.DataAprovacao.Month == mes && s.DataAprovacao.Year == ano)
                .OrderByDescending(s => s.ID)
                .OrderByDescending(s => s.DataIntegracao)
